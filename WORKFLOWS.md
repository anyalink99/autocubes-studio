...
The frame-locked pass prewarms lazy sections, advances browser animation time from the existing monotonic clock, verifies the real scroll position, normalizes configured lower-FPS embedded sources, and extracts their CFR frames into a reusable JPEG cache. It creates one persistent canvas beside every connected Framer video clone, disables native controls, and starts each source clock only when that source first enters the vertical preload zone. Keeping all horizontal/responsive clones alive prevents carousel handoff races without silently pre-rolling their media from capture frame zero. Each media carousel is snapped to a complete slide once and that same transform is reasserted for the whole capture; never recalculate the nearest slide from a moving autoplay phase. Every clone ancestor inside the list is kept visible and opaque; the outer carousel still clips distant slides, while the inherited virtual-clock phase cannot expose a transitional one-card state. DOM-local placement preserves clipping, scale, rounded corners, masks, and stacking. The page screenshot sequence is then encoded as CFR H.264.
