...
The frame-locked pass prewarms lazy sections, advances browser rAF and application timers from one inherited monotonic clock, verifies the real scroll position, normalizes configured lower-FPS embedded sources, and extracts their CFR frames into a reusable JPEG cache. Capture-internal readiness timeouts keep separate native timer handles, so deterministic page time cannot deadlock media loading. The pass creates one persistent canvas beside every connected Framer video clone, disables native controls, and starts each source clock only when that source first enters the vertical preload zone. Keeping all horizontal/responsive clones alive prevents carousel handoff races without silently pre-rolling their media from capture frame zero. Each media carousel is snapped to a complete slide once and that same transform is reasserted for the whole capture; never recalculate the nearest slide from a moving autoplay phase. Every clone ancestor inside the list is kept visible and opaque; the outer carousel still clips distant slides, while the inherited virtual-clock phase cannot expose a transitional one-card state. DOM-local placement preserves clipping, scale, rounded corners, masks, and stacking. Narrow native-verified stable-state locks prevent component-specific Framer transform flips, while monotonic transform tracks reject carousel reversals and oversized per-frame jumps. The page screenshot sequence is then encoded as CFR H.264.
...
`npm run make:flowline` runs all four stages. Capture warms assets in a disposable page, then records a fresh DOM so scroll-triggered reveals still run. Capture QA rejects backwards media-time steps, unbudgeted visible loop wraps, DOM transform/rotation/opacity jumps, missing reveal ranges, duplicate frames, excessive repeated embedded-media timestamps, freeze events, unexpected temporal discontinuities, wrong frame counts, and nominal/average FPS mismatches. Verified hard cuts inside source media may use the scenario's `maxTemporalSpikeEvents` budget; duplicate, freeze, and Flowline loop-wrap allowances remain zero. In Remotion, keep contiguous master footage inside one `OffthreadVideo` and do not reset camera scale at chapter-label boundaries. Render QA allows intentional static edit holds such as an end card but still verifies CFR. The reusable agent workflow is stored in `.agents/skills/autocubes-video-pipeline`.
