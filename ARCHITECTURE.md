...
Final live-site presentation passes use `tooling/capture/frame-locked.ts`, not Playwright `recordVideo`. The module prewarms lazy layout, moves browser rAF/timeouts/intervals onto one inherited monotonic virtual clock, keeps capture-internal readiness timeouts native, verifies scroll position, motion-normalizes configured lower-FPS media, builds a deterministic JPEG frame cache outside Chromium, and feeds the requested frame into a persistent canvas beside every connected Framer video clone. Media clocks start at first vertical activation rather than offscreen DOM discovery, and visible loop wraps fail by default. Each media carousel retains its first complete-slide transform for the full capture instead of resnapping from a moving autoplay phase, and all clone ancestors inside the list stay visible/opaque; the outer carousel clip keeps distant clones hidden while autoplay-phase handoff states disappear. Narrow stable-element locks reassert native-verified component states immediately before screenshots, and monotonic transform tracks reject carousel direction reversals or oversized steps. The DOM-local proxies otherwise inherit the site's clipping, transforms, masks, and stacking without repeated browser video seeks. The result is encoded from a CFR screenshot sequence and checked for object motion, direction, cadence, loop wraps, freezes, duplicates, and temporal discontinuities. Agent-assisted reel work uses the upstream [Remotion Agent Skills](https://github.com/remotion-dev/skills) together with the repository-owned `.agents/skills/autocubes-video-pipeline`; the upstream collection can be installed with `npx skills add remotion-dev/skills .`.
...
`npm run qa:motion` verifies distance-aware direction, curved cursor paths, exact target landing, ripple edits, and quality scoring. `npm run qa:smoke` verifies core export and API paths. `npm run qa:identity` stresses variant paging, zoom, and responsive geometry. `npm run qa:workflows` covers Motion Page Map, Timeline, and Capture Director, Identity Brand Kit and Carousel Builder, Documents blocks and export, and Studio Project Hub. `npm run qa:capture:flowline` rejects capture FPS drift, frame-count mismatches, DOM transform reversals/jumps, duplicate decoded frames, excessive repeated embedded-media timestamps, freezes, and unexpected consecutive-frame discontinuities. A scenario may budget verified source hard cuts with `maxTemporalSpikeEvents`; this never relaxes duplicate or freeze limits. `npm run qa:render:flowline` verifies the final CFR render while allowing intentional edit holds and cuts. `npm run check` runs the shared suites plus TypeScript, production build, and Remotion composition discovery.
