...
Final live-site presentation passes use `tooling/capture/frame-locked.ts`, not Playwright `recordVideo`. The module prewarms lazy layout in a disposable page, opens a fresh capture DOM so once-only in-view reveals remain intact, moves browser rAF/timeouts/intervals onto one inherited monotonic virtual clock, keeps capture-internal readiness timeouts native, verifies scroll position, motion-normalizes configured lower-FPS media, builds a deterministic JPEG frame cache outside Chromium, and feeds the requested frame into a persistent canvas beside every connected Framer video clone. Media clocks start at first vertical activation rather than offscreen DOM discovery, and visible loop wraps fail by default. The capture does not rewrite carousel transforms, component transforms, visibility, opacity, transitions, or animations. Observation-only continuity tracks instead reject direction reversals, translation/rotation/opacity jumps, and missing reveal ranges. DOM-local proxies inherit the site's clipping, transforms, masks, and stacking without repeated browser video seeks. The result is encoded from a CFR screenshot sequence and checked for object motion, direction, cadence, loop wraps, freezes, duplicates, and temporal discontinuities. Agent-assisted reel work uses the upstream [Remotion Agent Skills](https://github.com/remotion-dev/skills) together with the repository-owned `.agents/skills/autocubes-video-pipeline`; the upstream collection can be installed with `npx skills add remotion-dev/skills .`.
