...
Final live-site presentation passes use `tooling/capture/frame-locked.ts`, not Playwright `recordVideo`. The module prewarms lazy layout, advances a monotonic virtual browser clock, verifies scroll position, motion-normalizes configured lower-FPS media, builds a deterministic JPEG frame cache outside Chromium, and feeds the requested frame into a persistent canvas beside every connected Framer video clone. Each media carousel retains its first complete-slide transform for the full capture instead of resnapping from a moving autoplay phase, and all clone ancestors inside the list stay visible/opaque; the outer carousel clip keeps distant clones hidden while autoplay-phase handoff states disappear. The DOM-local proxies otherwise inherit the site's clipping, transforms, masks, and stacking without repeated browser video seeks. The result is encoded from a CFR screenshot sequence and checked for direction, cadence, freezes, duplicates, and temporal discontinuities. Agent-assisted reel work uses the upstream [Remotion Agent Skills](https://github.com/remotion-dev/skills) together with the repository-owned `.agents/skills/autocubes-video-pipeline`; the upstream collection can be installed with `npx skills add remotion-dev/skills .`.
