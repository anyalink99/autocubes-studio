...
Final live-site presentation passes use `tooling/capture/frame-locked.ts`, not Playwright `recordVideo`. The module prewarms lazy layout, advances a monotonic virtual browser clock, verifies scroll position, motion-normalizes configured lower-FPS media, samples remounted embedded-video decoders at output-frame midpoints into retained canvas proxies, encodes a CFR image sequence, and runs cadence plus temporal-continuity QA. Agent-assisted reel work uses the upstream [Remotion Agent Skills](https://github.com/remotion-dev/skills) together with the repository-owned `.agents/skills/autocubes-video-pipeline`; the upstream collection can be installed with `npx skills add remotion-dev/skills .`.
