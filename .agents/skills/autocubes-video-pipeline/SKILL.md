...
8. Treat object-state continuity as separate from frame cadence. Observe important reveal wrappers, phones, globes, and carousel lists with `transformContinuityTracks`. Reject excessive per-frame translation, rotation, scale, or opacity changes; for ticker/carousel lists also track the viewport coordinates of visible direct children so a one-card recycle jump cannot hide inside a smooth parent transform. Use minimum opacity/translation ranges to prove that expected scroll reveals actually ran. QA must watch the native state without changing it.
...
- Do not infer object continuity from global frame hashes. Validate known objects with transform continuity tracks and reject direction reversals, excessive single-frame displacement/rotation/opacity changes, visible descendant jumps, or missing reveal ranges.
...
- If an already approved final has one isolated compositor-only artifact, repair only the affected region from a verified adjacent clean frame using the measured scroll displacement. Never replace the full frame, never use this to waive repeated motion failures, preserve the original audio, and inspect the frame before/after the repair at full resolution.
