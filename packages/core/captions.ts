...
export const parseSrt = (source: string): CaptionEvent[] => source.trim().split(/\r?\n\s*\r?\n/).flatMap((block,index) => {
  ...
  return [{id:`caption-import-${Date.now()}-${index}`,label:`Текст ${index+1}`,text,textEn:text,textRu:text,at:from,duration:Math.max(.2,to-from),position:'bottom' as const,style:'boxed' as const,size:54,align:'center' as const,maxWidth:86,lineHeight:1.08,letterSpacing:-2.5,animation:'rise' as const}];
});
...
export const linesToCaptions = (source: string, startAt: number) => source.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((text,index) => {
  ...
  return {id:`caption-lines-${Date.now()}-${index}`,label:`Текст ${index+1}`,text,textEn:text,textRu:text,at:startAt+previousTime,duration,position:'bottom' as const,style:'boxed' as const,size:54,align:'center' as const,maxWidth:86,lineHeight:1.08,letterSpacing:-2.5,animation:'rise' as const};
});
