...
import {applyRecipe, applyRecommendedCaptureActions, arrangeFrames, buildDirectedCapturePlan, clamp, duplicateTimelineItem, formatEditorTime, migrateEditorProject, MotionRecipeId, moveTimelineItems, polishMotionProject, trimTimelineItem, updateFrameWithRipple} from '../../packages/core/editor-operations';
...
export const App = () => {
  ...
  return (
    <div className={`editor-app ${previewFocus?'preview-focus':''}`} style={{gridTemplateRows:`54px minmax(0,1fr) ${previewFocus?0:timelineHeight}px`}}>
      ...
      <Timeline project={project} currentTime={currentTime} selection={selection} pixelsPerSecond={zoom} onSeek={(time) => {setPlaying(false); setCurrentTime(time);}} onSelect={setSelection} onMoveItems={(track,updates)=>{setSelection({track,id:updates[0]?.id,ids:updates.map((item)=>item.id)});commit((draft)=>moveTimelineItems(draft,track,updates),`timeline-move:${track}:${updates.map((item)=>item.id).join(',')}`);}} onTrimItem={(track,id,at,duration,_edge,ripple)=>{setSelection({track,id,ids:[id]});commit((draft)=>trimTimelineItem(draft,track,id,at,duration,ripple),`timeline-trim:${track}:${id}`);}} onAdd={addItem} onToggleSnap={() => commit((draft) => {draft.snap = draft.snap === false; return draft;})} onZoom={setZoom} onSplit={splitSelected} onAddMarker={addMarker} onResizeHeight={(height)=>{setTimelineHeight(height);localStorage.setItem('motion-desk-timeline-height',String(height));}} />

      {notice ? <div className={`editor-toast ${notice.tone}`}>{notice.message}</div> : null}
      {showCaptureDirector?<CaptureDirector project={project} analysis={project.captureAnalysis} analyzing={analyzingPage} recording={job?.status==='running'&&job.kind==='capture'} onChangeUrl={(url)=>commit((draft)=>{draft.url=url;draft.captureAnalysis=undefined;return draft;})} onAnalyze={()=>void analyzeCaptureSource()} onBuild={buildCapturePlan} onPolish={(profile)=>commit((draft)=>polishMotionProject(draft,profile),'capture-polish')} onRecommendActions={(profile)=>commit((draft)=>applyRecommendedCaptureActions(draft,draft.captureAnalysis?.targets,profile),'capture-actions')} onChangeFrame={(id,patch)=>commit((draft)=>updateFrameWithRipple(draft,id,{...patch,...(patch.scrollY===undefined?{}:{scrollY:clamp(patch.scrollY,0,Math.max(0,draft.pageHeight-draft.viewport.height))})}),`capture-frame:${id}:${Object.keys(patch).join(',')}`)} onRecord={()=>{setShowCaptureDirector(false);void runJob('capture');}} onClose={()=>setShowCaptureDirector(false)}/>:null}
