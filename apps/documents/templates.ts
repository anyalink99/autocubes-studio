import {defaultEmployeeProfile, EmployeeProfileData} from './employee-profile';
...
export const templates:DocumentTemplate[] = [
  {
    templateId:'employee-profile', name:'Employee profile', short:'Team', category:'Operations', type:'employee profile',
    title:'Employee profile — team member', client:'Autocubes', project:'Team', status:'Published',
    intro:'A client-ready profile of an Autocubes specialist, their role, background, and production focus.',
    blocks:[], accent:'#ff4e2f', employeeProfile:structuredClone(defaultEmployeeProfile),
  },
...
export const createDocument=(templateId:string):StudioDocument=>{
  ...
  const now=new Date();
  if(source.employeeProfile){
    const title=`Профиль сотрудника — ${source.employeeProfile.name}`;
    const content={title,intro:source.employeeProfile.summary,client:'Autocubes',project:'Команда',type:'профиль сотрудника',status:'Черновик',blocks:[]};
    return {...structuredClone(source),...content,id:`doc-${Date.now()}`,date:now.toISOString().slice(0,10),updatedAt:now.toISOString(),versions:[],language:'ru',localized:{ru:content,en:{...content,title:`Employee profile — ${source.employeeProfile.name}`,type:'employee profile',status:'Draft'}}};
  }
