...
Video work combines the upstream [Remotion Agent Skills](https://github.com/remotion-dev/skills) with the project-specific [`autocubes-video-pipeline`](./.agents/skills/autocubes-video-pipeline/SKILL.md). The upstream skills provide Remotion composition, timing, media, and render practices. The local skill adds a shared virtual rAF/timer clock, disposable-page asset warmup plus a fresh reveal-preserving capture DOM, frame-locked browser capture, cached CFR/JPEG extraction for embedded media, first-visible media clocks, zero-by-default visible loop wraps, one persistent DOM-local canvas per Framer clone, native mask/stacking preservation, observation-only DOM transform/reveal QA, stable continuous Remotion playback, per-source direction/cadence checks, temporal-continuity QA, and the failure modes learned from production Autocubes reels.
